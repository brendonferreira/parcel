...
export default class BundleGraph {
  ...
  isAssetReferencedByDependant(bundle: Bundle, asset: Asset): boolean {
    ...
    this.traverseBundles((descendant, _, actions) => {
      if (
        descendant.type !== asset.type ||
        descendant.env.context !== asset.env.context
      ) {
        actions.skipChildren();
        return;
      }
      ...
      let similarSiblings = this.getSiblingBundles(descendant).filter(
        sibling =>
          sibling.type === asset.type &&
          sibling.env.context === asset.env.context,
      );
      if (similarSiblings.some(sibling => bundleHasReference(sibling))) {
        isReferenced = true;
        actions.stop();
        return;
      }
